	...
	pub(super) fn _events(&self) {
		self.wnd.on().wm_init_dialog({
			...
			move |_| {
				...
				self2.chk_rem_padding.set_check_state(gui::CheckState::Checked);
		...
		self.chk_rem_padding.on().bn_clicked({
			let self2 = self.clone();
			move || self2._enable_disable_rem_padding()
		});
		...
		self.btn_ok.on().bn_clicked({
			...
			move || {
				...
				let mut tags_cache = self2.tags_cache.try_borrow_mut()?;
				...
				for file in self2.files.iter() { // execute the chosen operations on each file
					...
					if self2.chk_rem_rg.is_checked() {
						self2._remove_replay_gain(&mut tag);
					}
					if self2.chk_prefix_year.is_checked() {
						if let Err(err) = self2._prefix_year(&mut tag, file) {
