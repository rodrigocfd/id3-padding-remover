...
using std::optional, std::wstring;
...
void DlgEdit::renderTextboxes() const
{
	for (auto&& field : _Fields) {
		optional<const id3::Frame*> pFrame;
		if (_pTags.size() == 1) {
		...
		lib::CheckRadio{this, field.chkId}.checkAndTrigger(pFrame.has_value());
		lib::NativeControl{this, static_cast<WORD>(field.chkId + 1)}
			.setText(pFrame.has_value() ? pFrame.value()->asText() : L"");
	}
}
