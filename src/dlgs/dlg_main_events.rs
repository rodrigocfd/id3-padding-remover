use winsafe::{self as w, cmd, co, evp, gui, prelude::*};

use super::{DlgEdit, DlgMain, LIST_COLS, ids};
use crate::msgbox;

impl DlgMain {
	pub(super) fn events(&self) {
		evp!(self, wm_init_dialog, on_init_dialog);
		evp!(self, wm_size, on_size);
		evp!(self, wm_init_menu_popup, on_init_menu_popup);
		cmd!(self, ids::MNU_FILE_OPEN, on_menu_file_open);
		cmd!(self, ids::MNU_FILE_EDIT, on_menu_file_edit);
		cmd!(self, ids::MNU_FILE_REMOVE, on_menu_file_remove);
		cmd!(self, ids::MNU_FILE_REWRITE, on_menu_file_rewrite);
		cmd!(self, ids::MNU_FILE_WRITE_TRACK_NO, on_menu_file_write_track_no);
		cmd!(self, ids::MNU_FILE_RENAME_TAT, on_menu_file_rename_tat);
		cmd!(self, ids::MNU_FILE_RENAME_AT, on_menu_file_rename_at);
		cmd!(self, ids::MNU_FILE_REMRG, on_menu_file_remove_rg);
		cmd!(self, ids::MNU_FILE_REMRGART, on_menu_file_remove_rg_art);
		cmd!(self, ids::MNU_FILE_ABOUT, on_menu_file_about);
		evp!(self, self.lst_files, lvn_item_changed, on_list_item_changed);
		evp!(self, self.lst_files, lvn_key_down, on_list_key_down);
		evp!(self, self.lst_files, nm_dbl_clk, on_list_dbl_clk);
		evp!(self, self.lst_files, lvn_delete_item, on_list_delete_item);
		evp!(self, self.lst_files.header().unwrap(), hdn_item_click, on_list_header_click);

		let self2 = self.clone();
		self.drop_target.Drop(move |d, _, _, _| {
			let mut fmt = w::FORMATETC::default();
			fmt.cfFormat = co::CF::HDROP;
			fmt.dwAspect = co::DVASPECT::CONTENT;
			fmt.tymed = co::TYMED::HGLOBAL;

			let medium = unsafe { d.GetData(&fmt)? };
			let hglobal = unsafe { medium.ptr_hglobal().unwrap() };
			let ptr_lock = hglobal.GlobalLock()?;
			let hdrop = unsafe { w::HDROP::from_ptr(ptr_lock.as_ptr() as _) };
			let dropped_paths = hdrop.DragQueryFile()?.collect::<w::SysResult<Vec<_>>>()?;

			self2.add_files_to_list(&dropped_paths)?;
			Ok(())
		});
	}

	fn on_init_dialog(&self, _: w::msg::WmInitDialog) -> w::AnyResult<bool> {
		let lv = &self.lst_files;
		self.update_num_files_in_titlebar(lv.items().count())?;

		lv.image_list(co::LVSIL::SMALL)?
			.add_icons_from_shell(&["mp3"])?;
		lv.set_extended_style(true, co::LVS_EX::FULLROWSELECT);
		lv.context_menu()
			.unwrap()
			.SetMenuDefaultItem(w::IdPos::Id(ids::MNU_FILE_EDIT))?;

		let header_cols = lv.header().unwrap().items();
		LIST_COLS
			.iter()
			.try_for_each(|(title, cx, txt_just, _)| -> w::SysResult<_> {
				let col = lv.cols().add(*title, gui::dpi_x(*cx))?; // add the columns
				txt_just.map(|just| {
					header_cols.get(col.index()).set_justify(just); // text justification if specified
				});
				Ok(())
			})?;

		header_cols.get(0).set_arrow(gui::HeaderArrow::Asc); // initially 1st col, ascending
		lv.cols().get(0).set_width_to_fill()?;

		self.wnd.hwnd().RegisterDragDrop(&self.drop_target)?;
		Ok(true)
	}

	fn on_size(&self, p: w::msg::WmSize) -> w::AnyResult<()> {
		if p.request != co::SIZE_R::MINIMIZED {
			self.lst_files.cols().get(0).set_width_to_fill()?;
		}
		Ok(())
	}

	fn on_init_menu_popup(&self, p: w::msg::WmInitMenuPopup) -> w::AnyResult<()> {
		if p.hmenu == self.lst_files.context_menu().unwrap() {
			let has_sel = self.lst_files.items().selected_count() >= 1;
			[
				ids::MNU_FILE_EDIT,
				ids::MNU_FILE_REMOVE,
				ids::MNU_FILE_RENAME_TAT,
				ids::MNU_FILE_RENAME_AT,
				ids::MNU_FILE_REWRITE,
				ids::MNU_FILE_WRITE_TRACK_NO,
				ids::MNU_FILE_REMRG,
				ids::MNU_FILE_REMRGART,
			]
			.into_iter()
			.try_for_each(|id| {
				p.hmenu
					.EnableMenuItem(w::IdPos::Id(id), has_sel)
					.map(|_| ())
			})?;
		}
		Ok(())
	}

	fn on_menu_file_open(&self) -> w::AnyResult<()> {
		let fod = w::CoCreateInstance::<w::IFileOpenDialog>(
			&co::CLSID::FileOpenDialog,
			None::<&w::IUnknown>,
			co::CLSCTX::INPROC_SERVER,
		)?;

		fod.SetOptions(
			fod.GetOptions()?
				| co::FOS::FORCEFILESYSTEM
				| co::FOS::FILEMUSTEXIST
				| co::FOS::ALLOWMULTISELECT,
		)?;

		fod.SetFileTypes(&[("MP3 audio files", "*.mp3"), ("All files", "*.*")])?;
		fod.SetFileTypeIndex(1)?;

		if fod.Show(self.wnd.hwnd())? {
			self.add_files_to_list(
				&fod.GetResults()?
					.iter()?
					.map(|shi| shi?.GetDisplayName(co::SIGDN::FILESYSPATH))
					.collect::<w::HrResult<Vec<_>>>()?,
			)?;
		}
		Ok(())
	}

	fn on_menu_file_edit(&self) -> w::AnyResult<()> {
		if self.lst_files.items().selected_count() == 0 {
			return Ok(()); // Enter key will hit here even if there are no selected items
		}

		let cloned_sel_tags = self
			.lst_files
			.items()
			.iter_selected()
			.map(|sel_item| {
				let rc_tag = sel_item.data();
				let cloned_tag = rc_tag.try_borrow()?.clone();
				Ok(cloned_tag)
			})
			.collect::<w::AnyResult<Vec<_>>>()?; // deep copy of selected tags

		// Show the modal window, which will take ownership of the cloned tags.
		// If user clicked OK, returns Some with the modified tags.
		if let Some(edited_tags) = DlgEdit::show(&self.wnd, cloned_sel_tags)? {
			self.lst_files.set_redraw(false);

			self.lst_files
				.items()
				.iter_selected() // the items order should be the same
				.zip(edited_tags.into_iter())
				.try_for_each(|(sel_item, mut edited_tag)| -> w::AnyResult<_> {
					edited_tag.save_to_file(&sel_item.text(0))?;
					sel_item.data().replace(edited_tag); // replace the tag currently stored in the item
					Self::render_tag(sel_item)?; // update the listview with the new values
					Ok(())
				})?;

			self.lst_files.set_redraw(true);
		}

		Ok(())
	}

	fn on_menu_file_remove(&self) -> w::AnyResult<()> {
		self.lst_files.items().delete_selected()?;
		Ok(())
	}

	fn on_menu_file_rewrite(&self) -> w::AnyResult<()> {
		let count = self.lst_files.items().selected_count();
		let ss = if count == 1 { "" } else { "s" };
		let content = format!("Rewrite the tag in {} file{}?", count, ss);

		if msgbox::ask(&self.wnd, &format!("Rewrite file{}", ss), None, &content, "&Rewrite")? {
			self.lst_files
				.items()
				.iter_selected()
				.try_for_each(|sel_item| -> w::AnyResult<_> {
					{
						let rc_tag = sel_item.data(); // retrieve tag saved in the listview item
						let mut tag = rc_tag.try_borrow_mut()?;
						tag.save_to_file(&sel_item.text(0))?; // save to MP3 file
					}
					Self::render_tag(sel_item)?; // padding will be set to zero, if any
					Ok(())
				})?;
		}
		Ok(())
	}

	fn on_menu_file_write_track_no(&self) -> w::AnyResult<()> {
		self.lst_files
			.items()
			.iter_selected()
			.enumerate()
			.try_for_each(|(idx, sel_item)| -> w::AnyResult<_> {
				{
					let rc_tag = sel_item.data();
					let mut tag = rc_tag.try_borrow_mut()?;
					tag.set_editable_string("TRCK", &(idx + 1).to_string())?;
					tag.save_to_file(&sel_item.text(0))?;
				}
				Self::render_tag(sel_item)?;
				Ok(())
			})?;

		Ok(())
	}

	fn on_menu_file_rename_tat(&self) -> w::AnyResult<()> {
		self.rename(true)
	}

	fn on_menu_file_rename_at(&self) -> w::AnyResult<()> {
		self.rename(false)
	}

	fn on_menu_file_remove_rg(&self) -> w::AnyResult<()> {
		self.remove_rg_art(false)
	}

	fn on_menu_file_remove_rg_art(&self) -> w::AnyResult<()> {
		self.remove_rg_art(true)
	}

	fn on_menu_file_about(&self) -> w::AnyResult<()> {
		let exe_name = w::HINSTANCE::NULL.GetModuleFileName()?;
		let hversion = w::HVERSIONINFO::GetFileVersionInfo(&exe_name)?;
		let version_parts = hversion.version_info()?.dwFileVersion();

		let content = format!(
			"Version {}.{}.{}\n\
				Written in Rust with WinSafe library.\n\n\
				{}",
			version_parts[0],
			version_parts[1],
			version_parts[2],
			hversion.str_val(hversion.langs_and_cps()?[0], "LegalCopyright")?,
		);

		msgbox::info(&self.wnd, "About", Some("ID3 Fit"), &content)?;
		Ok(())
	}

	fn on_list_item_changed(&self, _: &w::NMLISTVIEW) -> w::AnyResult<()> {
		self.update_num_files_in_titlebar(self.lst_files.items().count())?;
		Ok(())
	}

	fn on_list_key_down(&self, p: &w::NMLVKEYDOWN) -> w::AnyResult<()> {
		if p.wVKey == co::VK::DELETE {
			// On DEL key, remove selected files from the list.
			self.lst_files.items().delete_selected()?;
		} else if p.wVKey == co::VK::RETURN {
			// On Enter key, edit the selected tags.
			self.wnd
				.hwnd()
				.SendCommand(w::AccelMenuCtrl::Menu(ids::MNU_FILE_EDIT));
		}
		Ok(())
	}

	fn on_list_dbl_clk(&self, _: &w::NMITEMACTIVATE) -> w::AnyResult<()> {
		self.wnd
			.hwnd()
			.SendCommand(w::AccelMenuCtrl::Menu(ids::MNU_FILE_EDIT));
		Ok(())
	}

	fn on_list_delete_item(&self, _: &w::NMLISTVIEW) -> w::AnyResult<()> {
		// Notification is sent before the list is updated.
		self.update_num_files_in_titlebar(self.lst_files.items().count() - 1)?;
		Ok(())
	}

	fn on_list_header_click(&self, p: &w::NMHEADER) -> w::AnyResult<()> {
		let new_col = p.iItem as u32; // index of column clicked by user
		let (cur_col, cur_is_asc) = self.cur_sort.get(); // read current sort state
		let new_is_asc = new_col != cur_col || !cur_is_asc; // will sorting be ascending?
		self.cur_sort.set((new_col, new_is_asc)); // save new sort state

		let cols = self.lst_files.header().unwrap().items();
		cols.iter()?.for_each(|col| {
			col.set_arrow(gui::HeaderArrow::None); // remove arrow from all listview cols
		});
		let new_arrow = if new_is_asc { gui::HeaderArrow::Asc } else { gui::HeaderArrow::Desc };
		cols.get(new_col).set_arrow(new_arrow); // draw arrow in current col

		self.sort_list()?;
		Ok(())
	}
}
